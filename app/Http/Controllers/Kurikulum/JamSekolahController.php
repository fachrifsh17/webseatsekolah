...
class JamSekolahController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('log.admin')->only(['update', 'store']);
    }
    ...
    public function store(StoreJamSekolahRequest $request): JsonResponse
    {
        ...
        // 🔍 Cek duplikat jadwal di tahun ajaran aktif
        ...
        if ($exists) {
            return response()->json([
                'success' => false,
                'message' => "Gagal: Jadwal untuk hari {$validated['hari']} jam ke {$validated['jam_ke']} sudah ada di tahun ajaran ini."
        ...
        try {
            ...
            Log::error('JamSekolah store error', [
                'error' => $e->getMessage()
            ]);
    ...
    public function update(UpdateJamSekolahRequest $request, JamSekolah $jamSekolah): JsonResponse
    {
        ...
        // 🔍 Cek duplikat jadwal di tahun ajaran aktif (abaikan record yang sedang diupdate)
        $exists = JamSekolah::where('tahun_ajaran_id', $tahunAktif->id)
            ->where('hari', $validated['hari'])
            ->where('jam_ke', $validated['jam_ke'])
        ...
        try {
            ...
            Log::error('JamSekolah update error', [
                'jam_sekolah_id' => (string) $jamSekolah->id,
                'error'          => $e->getMessage()
            ]);
    ...
    public function destroy(JamSekolah $jamSekolah): JsonResponse
    {
        ...
        try {
            ...
            Log::error('JamSekolah destroy error', [
                'jam_sekolah_id' => (string) $jamSekolah->id,
                'error'          => $e->getMessage()
            ]);
