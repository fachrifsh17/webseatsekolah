   ...
   public function listKelas(Request $request): JsonResponse
{
    try {
        ...
        // 3. Ambil daftar kelas pada TA tersebut
        ...
        // 4. Map status presensi
        $dataWithStatus = $kelas->map(function ($item) use ($tanggal, $taActive) {
            // Cek apakah minimal sudah ada satu siswa di kelas ini yang diabsen pada tanggal tsb
        ...
        // 5. TAMBAHAN: Filter berdasarkan status (opsional)
    ...
    public function listSiswaPresensi(Request $request): JsonResponse
{
    ...
    try {
        ...
        // Cek apakah sudah ada data absen di kelas ini pada tanggal tsb
        ...
        $collection = $siswa->map(function ($item) {
            ...
            return [
                ...
                'nisn'     => $item->nisn,
                'status'   => $presensiExisting?->status ?? null, // null jika belum absen
        ...
        return response()->json([
            'success' => true, 
            'info'    => [
                'presensi_id'      => $siswa->first()?->presensi->first()?->id ?? null, // ID salah satu record untuk tanda
        ...
        try {
            ...
            $results = DB::transaction(function () use ($dataInput, $tanggalInput, $taActive, $requestKelasId) {
                ...
                foreach ($dataInput as $item) {
                    ...
                    $siswaQuery = Siswa::with('kelas')->where('id', $item['siswa_id'])->where('is_active', true);
                    if ($requestKelasId) {
                        $siswaQuery->where('kelas_id', $requestKelasId);
                    }
            ...
            if ($request->has('data_presensi')) {
                ...
                $results = DB::transaction(function () use ($request, $tanggal, $requestKelasId) {
                    ...
                    foreach ($request->input('data_presensi') as $item) {
                        ...
                        if ($requestKelasId) {
                            $query->whereHas('siswa', fn($q) => $q->where('kelas_id', $requestKelasId));
                        }
